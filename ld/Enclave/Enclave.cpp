...
extern "C"
{
	...
	size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream)
	{
		...
		int preturn;
		fwrite_ocall(&preturn, ptr, size, 1, stream,&in_errno);
		return preturn;
	}
	...
	{
		static char dcgettext_tmp[200];
		dcgettext_ocall(domainname,msgid,category,dcgettext_tmp);
                printf("log %s\n","dcgettext");
		return dcgettext_tmp;
	}
	// getenv
	...
	{
		int preturn;
		fflush_ocall(&preturn,stream,&in_errno);
		return preturn;
                printf("log %s\n","fflush");
	}
	// fdopen
	...
	{
		static char bindtextdomain_tmp[200];
		int preturn;
		bindtextdomain_ocall(&preturn,domainname,dirname,bindtextdomain_tmp);
		if(preturn) 
		{
			return nullptr;
		}
	...
	int posix_spawn(pid_t *pid, const char *path,const void *file_actions,const void *attrp,char *const argv[], char *const envp[])
	{
		...
		char *argv_all = (char *)calloc(10000,1);
		char *env_all = (char *)calloc(10000,1);
	...
	{
		int preturn;
		setupterm_ocall(&preturn,term,fd,errret);
		printf("log setupterm:%s\n",term);
		return preturn;
	}
	...
	*/
	...
	{
		printf("enter ecall_compile\n");
		//printf("Inter the enclave\n");
		//errno_addr = *e_addr;
		...
		memset(env_use,'\0',sizeof(env_use));
		memset(argv_use,'\0',sizeof(argv_use));
		int length=0;
		...
		{
			env_use[i]=all_env+length;
			env_use[i][length_env[i]]='\0';
			length+=length_env[i]+1;
		}
		// //printf("**********************************\n");
		// for(int i=0;i<cnt_argv;i++)
		// {
		...
		// }
		// printf("****************************\n");
		main(cnt_argv,argv_use,env_use);
	}
	...
	char *getenv(const char *name)
	{
		...
		printf("getenv_before name = %s\n",name);
		getenv_ocall(&preturn,name, (void **)&ret);
		printf("getenv %s = %s\n",name,ret);
                
		// if(strcmp("LLD_REPRODUCE",name) == 0)
		// {
		// 	asm("int3");
		// }
		if(preturn  == 1)
	...
	int tigetnum(char *capname)
	{
		...
		int preturn;
		tigetnum_ocall(&preturn,capname,&in_errno);
		printf("tigetnum capname:%s return:%d\n",capname,preturn);
		if(preturn!=-1&&preturn!=-2&&preturn !=0&&preturn !=1)
		{
			Iago();
		}
		return preturn;
	...
	{
		int preturn;
		fnmatch_ocall(&preturn,pattern,string,flags);
		return preturn;
		printf("log %s\n","fnmatch");
	}
	...
	{
		static char textdomain_tmp[200];
		int preturn;
		textdomain_ocall(&preturn,domainname,textdomain_tmp);
		if(preturn==0)
		{
			return nullptr;
		}
	...
	pid_t wait4(int pid, int *wstatus, int options,
	...
	int mkstemps(char *template1,int suffixlen)
	{
		printf("log %s\n","mkstemps");
	}
	int _IO_putc(const char *str)
	{
		printf("log %s\n","_IO_putc");
	}
