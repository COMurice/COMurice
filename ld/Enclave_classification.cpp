...
// __printf_chk
	...
	char *getenv(const char *name)
	{
		...
		getenv_ocall(&preturn,name, getenv_tmp);
		if (preturn == 0) {
			return nullptr;
		}
		return getenv_tmp;
...
//__ctype_b_loc
	...
	ssize_t readlink(char *path, char *buf, size_t bufsize)
	{
                printf("log %s\n","readlink");
		int preturn;
		readlink_ocall(&preturn, path, buf, bufsize);
		return preturn;
	}
	...
	int posix_spawn(pid_t *pid, const char *path,const posix_spawn_file_actions_t *file_actions,const struct  posix_spawnattr_t *attrp,char *const argv[], char *const envp[])
	{
                printf("log %s\n","posix_spawn");
	...
	// __iswctype_l
	int __iswctype_l(wint_t c,wctype_t d__towlower_lesc,_locale_t locale)
	{
                printf("log %s\n","__iswctype_l");
		return 0;
	}
	...
	int closedir(DIR *dir)
	{
                printf("log %s\n","closedir");
		// int preturn;
		// closedir_ocall(&preturn, (void *)dir);
		// return preturn;
	}	
	...
	{
                printf("log %s\n","readdir");
		// static struct dirent readdir_tmp;
		// int preturn;
		// readdir_ocall(&preturn,(void *)dir, (void *)&readdir_tmp);
		// if (preturn)
		// 	return &readdir_tmp;
		return nullptr;
	}
	...
	DIR *opendir(const char *name)
	{
                printf("log %s\n","opendir");
		// int preturn;
		// static DIR opendir_tmp;
		// opendir_ocall(&preturn,name, (void *)&opendir_tmp);
		// if (preturn)
		// 	return &opendir_tmp;
		return nullptr;
	}	
	...
	{
                printf("log %s\n","fopen");
		// FILE **fopen_tmp = (FILE **)malloc(sizeof(FILE *));
		// fopen_ocall(filename, mode, (void *)fopen_tmp);
		// return *(FILE **)fopen_tmp;
		return nullptr;
	}
	// __fprintf_chk
	...
	*/
            ...
            void ecall_compile()
	{
		...
		char *s = "/home/lab/sgxsdk/SampleCode/SGX_LLVM_0121/app";
		char *s2 = "a=b";
		char *tmp_argv[2];
		tmp_argv[0] = s;
		tmp_argv[1] = nullptr;
