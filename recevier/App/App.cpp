...
#define true 1
#define false 0
# define TOKEN_FILENAME   "/home/lab/panda/memory-resident/recevier/enclave.token"
# define ENCLAVE_FILENAME  "/home/lab/panda/memory-resident/recevier/enclave.signed.so"
using namespace std;
...
void local_attestaion()
{
	...
	uint32_t ret_status;	
	 // shared memory
	key_t key = ftok("/home/lab", 1);
	printf("%d\n",key);
	int shmid = shmget(key, 1024, 0666|IPC_CREAT);
	char *str = (char*)shmat(shmid, (void*)0, 0);
	printf("[TEST IPC] Sending to Enclave2: Hello from Enclave\n");
	strncpy(str, "Hello from Enclave5\n", 20);
	shmdt(str);
	do
	{
		printf("[START] Testing create session between Enclave1 (Initiator) and Enclave2 (Responder)\n");
		status = test_create_session(global_eid, &ret_status,global_eid , 0);
...
int SGX_CDECL  main(int argc, char *argv[], char *env[])
{
	...
	int updated = 0;
	int length_env[1000],length_argv[1000];
	char all_env[1000000];
	...
	if (ret != SGX_SUCCESS)
	{
		puts("enclave create error!");
	}
	int recive;
	
	char buffer[1000000]={0};
	recive = socket(AF_INET, SOCK_STREAM, 0);
	struct sockaddr_in address;
	address.sin_family = AF_INET;
	address.sin_addr.s_addr = INADDR_ANY;
	address.sin_port = htons(8080);
	int opt=1;
	setsockopt(recive,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt)); // reuse the addr.
	bind(recive, (struct sockaddr *)&address, sizeof(address));
	...
	while(true)
	{
		...
		while(1)
		{
			printf("accpet the message from clang\n");
			int recv_clang = recv(new_socket,all_argv,1000000,0);
			if(recv_clang > 0) break;
		}
		// int shmid=shmget(1234567, 1024, 0666|IPC_CREAT);
		// char *cwd = (char *)shmat(shmid,(void*)0,0);
		// chdir(cwd);
		// shmdt(cwd);
		ecall_compile(global_eid,cnt_argv,cnt_env,length_argv,length_env,all_argv,all_env);
		send(new_socket , "sucessed" , 8, 0 );
		close(new_socket);
...
void posix_spawn_ocall(int * preturn,int * pid,const char *path,const void *file_actions,const void *attrp,void *_argv,void *_envp,int argc,int envc)
{
	...
	// 	shared_memory=shmat(shmid_second,NULL,0);
	// }
	// printf("**************************************************posix_spawn_varible\n");
	// printf("pid=%d\n",*pid);
	// printf("path=%s\n",path);
	// for(int i=0;i<100;i++)
	// {
	// 	printf("outside argv %s\n",argv_use[i]);
	// }
	// for(int i=0;i<100;i++)
	// {
	// 	printf("outside env_use %s\n",env_use[i]);
	// }

	int sock = 0, valread;
	struct sockaddr_in serv_addr;
	char buffer_argv[100000] = {0};
	char buffer_env[100000]={0};
	sock = socket(AF_INET, SOCK_STREAM, 0);
	serv_addr.sin_family = AF_INET;
	serv_addr.sin_port = htons(8081);
	int rc = fcntl(sock, F_SETFL, O_NDELAY); //set socket into no-blocking model
	...
	if(!(path[l-1]=='d'&&path[l-2]=='l'))
	{
		...
		//*preturn = posix_spawn(pid,path,NULL,NULL,argv_use,env_use);
		//printf("argv = %s\n",argv);
		memcpy(buffer_argv,argv,argc);
		// printf("buffer = %s\n",buffer_argv);
		send(sock , buffer_argv , 100000,0);
		memcpy(buffer_env,envp,envc);
		// printf("send the second result\n");
		send(sock,buffer_env,100000,0);
		...
		while(1)
		{
			int flag = recv(sock,recv_data,10,0);  
			time_t end_time = time(NULL);
			int l = strlen(recv_data);
			if(l>0) break;
			if(end_time-start_time>30)  //represent compile_clang crash please resend the message
			{
				// printf("recevier_clang\n");
				close(sock);
				sock = socket(AF_INET, SOCK_STREAM, 0);
				while (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) ;  //reconnet because compie_clang crash
				// printf("recevier_clang next\n");
				send(sock , buffer_argv , 100000,0);
				send(sock,buffer_env,100000,0);
				start_time = end_time;
			}
		...
		*preturn = 321;  //represent the use of compile SGX_llvm
	}	
	else
	{
		*preturn = posix_spawn(pid,path,NULL,NULL,argv_use,env_use);
...
// fwrite
void fwrite_ocall(int *preturn,const void *ptr, size_t size, size_t nmemb, void *stream){
...
// fflush
void fflush_ocall(int *preturn,void *stream){
