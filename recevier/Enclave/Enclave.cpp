...
extern "C"
{
	...
	*/
        ...
        void ecall_compile(int cnt_argv,int cnt_env,int *length_argv,int *length_env,char *all_argv,char *all_env)
	{
		...
		for(int i = 0;i<mmap_index;i++)
		{
			// if(memcmp(mmap_addr[i],mmap_bufaddr[i],mmap_len[i] != 0 ))
			// {
			// 	printf("flength:%d       99999999999999999999999999\n",mmap_len[i]);
			// 	exit(0);
			// }
			// else
			// {
			// 	printf("flength:%d       888888888888888888\n",mmap_len[i]);
			// }
			// if(mmap_bufaddr[i])
			// 	free(mmap_bufaddr[i]);
			// if(mmap_addr[i])
