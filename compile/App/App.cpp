...
#define true 1
#define false 0
# define TOKEN_FILENAME   "/home/lab/panda/memory-resident/compile/enclave.token"
# define ENCLAVE_FILENAME  "/home/lab/panda/memory-resident/compile/enclave.signed.so"
using namespace std;
...
int SGX_CDECL  main(int argc, char *argv[], char *env[])
{
	...
	FD_SET(recive, &set); /* add our file descriptor to the set */
	int new_socket;
	while(true)
	{
		// if(compiled_file>=300)
		// {
		...
		printf("compile_main\n");
		int rv = select(recive + 1, &set, NULL, NULL, &timeout);
		if(rv==0)
		{
			printf("timeout occurred (40 second) \n"); /* a timeout occurred */
			exit(1);
		}
		new_socket = accept(recive, (struct sockaddr *)&address, (socklen_t*)&addrlen);
		// printf("blocking in compile step\n");
		memset(all_env,0,sizeof(all_env));
		memset(all_argv,0,sizeof(argv));
		int read_clang_argv=recv(new_socket,all_argv,100000,0);
		int read_clang_env=recv(new_socket,all_env,100000,0);
		// printf("all_argv = %s\n",all_argv);	
		// printf("all_env= %s\n",all_env);
		...
		// chdir(cwd);
		// shmdt(cwd);
		ecall_compile(global_eid,100000,100000,length_argv,length_env,all_argv,all_env);
...
void posix_spawn_ocall(int * preturn,int * pid,const char *path,const void *file_actions,const void *attrp,void *_argv,void *_envp,int argc,int envc)
{
	...
	if(!(path[l-1]=='d'&&path[l-2]=='l'))
	{
		...
		memcpy(argv,buffer,argc);
		send(sock , buffer , argc,0);
		char recv_data[5]={0};
